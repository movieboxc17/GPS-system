// Robust: run only after DOM is ready
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!mainContent || !journeyList || !colorLegend || !totalStats || !toggleAllBtn || !mapDiv) {
  ...
  fetch('Journeys/Data1.json')
    .then(r => {
      if (!r.ok) throw new Error('Failed to load journeys');
      return r.json();
    })
    .then(journeys => {
      if (!Array.isArray(journeys) || journeys.length === 0) {
        journeyList.innerHTML = '<div>Inga resor hittades.</div>';
        return;
      }
      ...
      colorLegend.innerHTML = `<button id='legendToggleBtn' style='margin-right:8px;'>Visa/Dölj färglegend</button><span id='legendItems' style='display:flex;'></span>`;
      ...
      journeys.forEach((j, idx) => {
        const color = colors[idx % colors.length];
        const points = (j.data && j.data.journey) ? j.data.journey.map(p => [p.latitude, p.longitude]).filter(pt => pt[0] && pt[1]) : [];
        ...
        card.innerHTML = `<label><input type='checkbox' class='journey-toggle' data-idx='${idx}' checked> <b class='journey-name' data-idx='${idx}'>${j.name || 'Resa'}</b></label>
          ...
          ${points.length === 0 ? '<div style="color:#e55934;font-size:13px;">Ingen GPS-data för denna resa.</div>' : ''}`;
        journeyList.appendChild(card);
      });
      // Total stats
      totalStats.innerHTML = `Totalt: ${(totalDistance/1000).toFixed(2)} km | Tid: ${formatTime(totalDuration)} | Punkter: ${totalPoints}`;
    ...
    .catch(err => {
      journeyList.innerHTML = `<div style='color:#e55934;'>Kunde inte ladda resor: ${err.message}</div>`;
    });
  ...
  mapDiv.style.minHeight = window.innerWidth < 540 ? '220px' : '320px';
  ...
  window.showJourneyModal = function(journey, color) {
  ...
  window.drawChart = function(canvasId, data, label, color) {
    if (!window.Chart) {
      const script = document.createElement('script');
      script.src = 'https://cdn.jsdelivr.net/npm/chart.js';
      script.onload = () => window.drawChart(canvasId, data, label, color);
