// upload.js
window.initJourneys = function() {
  const MAP_CENTER = [59.3293, 18.0686];
  ...
  function addJourneyToMap(journey, name, color) {
    ...
    if (!latlngs.length || !latlngs[0]) return;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('fetchLocalBtn').onclick = function() {
      const url = '/Journeys/all_journeys.json';
      fetch(url)
        ...
        .then(data => {
          ...
          if (Array.isArray(data)) {
            data.forEach((item, idx) => {
              ...
              addJourneyToMap(journey, item.name || `Journey ${idx+1}`, getColor(idx));
              document.getElementById('journeyList').innerHTML += `<div class='journey-item'><b>${item.name || `Journey ${idx+1}`}</b> (${journey.length} pts)</div>`;
            });
